...
def run_single_combo(params):
    W_OL, W_OM, W_OH = params
    start_time = time.time()
    print(f"Running combo: W_OL={W_OL}, W_OM={W_OM}, W_OH={W_OH}")
    ...
    print(f"Finished W_OL={W_OL}, W_OM={W_OM}, W_OH={W_OH} in {elapsed_time:.2f} sec")
    return result_df
# %%
# -------------------- Parameters and Setup --------------------

W_OL_values = [1, 2 , 3, 4, 5]
W_OM_values = [1, 2 , 3, 4, 5]
W_OH_values = [1, 2 , 3, 4, 5]
...
def main():
    param_grid = list(itertools.product(W_OL_values, W_OM_values, W_OH_values))
    print("Start")
    with Pool() as pool:
        print("again")
        results = pool.map(run_single_combo, param_grid)
    final_df = pd.concat(results, ignore_index=True)
    final_df.to_csv("all_weight_combos_results.csv", index=False)
    return final_df
